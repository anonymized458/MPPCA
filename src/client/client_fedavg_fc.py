    ...
    def train(self, is_print=False):
        ...
        for iteration in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                images, labels = images.reshape(-1, 28 * 28).to(self.device), labels.to(self.device)
                self.net.zero_grad()
                # optimizer.zero_grad()
    ...
    def eval_test(self):
        ...
        with torch.no_grad():
            for data, target in self.ldr_test:
                data, target = data.reshape(-1, 28*28).to(self.device), target.to(self.device)
