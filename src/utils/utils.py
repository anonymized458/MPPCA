...
from .datasets import MNIST_truncated, MNIST_rotated, CIFAR10_truncated, CIFAR10_rotated, CIFAR100_truncated, \
    ...
    ImageFolder_custom, USPS_truncated
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4, local_view=False):
    ...
    elif partition == "professional1":
        # 十个用户拥有标签0，剩下的90个用户iid
        net_dataidx_map = {i: np.ndarray(0, dtype=np.int64) for i in range(n_parties)}
        idx0 = np.where(y_train == 0)[0]
        np.random.shuffle(idx0)
        split = np.array_split(idx0, (n_parties // 10) * 3)
        for j in range(n_parties // 10):
            net_dataidx_map[j] = np.append(net_dataidx_map[j], split[j])
        for i in range(1,10):
            idx_k = np.where(y_train == i)[0]
            np.random.shuffle(idx_k)
            split = np.array_split(idx_k, (n_parties - n_parties // 10))
            for j in range(n_parties - n_parties // 10):
                net_dataidx_map[j + n_parties // 10] = np.append(net_dataidx_map[j + n_parties // 10], split[j])

    elif partition == "professional2":
        # 十个用户拥有标签01，剩下的90个用户iid
        net_dataidx_map = {i: np.ndarray(0, dtype=np.int64) for i in range(n_parties)}
        for i in range(2):
            idx0 = np.where(y_train == i)[0]
            np.random.shuffle(idx0)
            split = np.array_split(idx0, (n_parties // 10) * 3)
            for j in range(n_parties // 10):
                net_dataidx_map[j] = np.append(net_dataidx_map[j], split[j])
    ...
    elif partition == "label-replaced":
        net_dataidx_map = {i: np.ndarray(0, dtype=np.int64) for i in range(n_parties)}
        # idx0 = np.where(y_train == 0)[0]
        # for index in idx0:
        #     y_train[index] = 7
        # np.random.shuffle(idx0)
        # split = np.array_split(idx0, n_parties // 10)
        # for j in range(n_parties // 10):
        #     net_dataidx_map[j] = np.append(net_dataidx_map[j], split[j])
        # 把前20个的1和9互换
        for i in range(10):
