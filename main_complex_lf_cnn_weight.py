...
for time in range(args.times):
    ...
    for idx in range(args.num_users):
        ...
        if idx < args.flipnum:
            for index in range(len(train_ds_local.target)):
                if train_ds_local.target[index] == 0:
                    train_ds_local.target[index] = 5
                elif train_ds_local.target[index] == 1:
                    train_ds_local.target[index] = 5
                elif train_ds_local.target[index] == 2:
                    train_ds_local.target[index] = 5
                elif train_ds_local.target[index] == 3:
                    train_ds_local.target[index] = 5
                elif train_ds_local.target[index] == 4:
                    train_ds_local.target[index] = 5
    ...
    for iteration in range(args.rounds):
        ...
        for i in range(len(w_locals2)):
            ...
            for k in w_locals2[i].keys():
                w_i[k] = w_locals2[i][k] - w_locals_before[i][k].to(args.device)
            w_local_delta.append(copy.deepcopy(w_i))

        fed_avg_freqs = cal_freq_by_layer_pca_complex(w_local_delta, args.alpha, args.sigma)
        # record weights
        ...
        if not len(bad_weights) == 0:
            bad_weights_iter.append(np.mean(bad_weights))
    ...
    bad_weights_round.append(np.mean(bad_weights_iter))
    ...
    with open(path + str(args.trial) + '_fnum=' + str(args.flipnum) + '_sigma=' + str(
            args.sigma) + '_results_record_of_weight.txt',
              'w') as text_file:
    ...
    csv_test.to_csv(path + 'times' + times + '_fnum=' + str(args.flipnum) + '_weight_record.csv')
