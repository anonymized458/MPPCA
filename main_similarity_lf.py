    ...
    if idx < args.flipnum:
        for index in range(len(train_ds_local.target)):
            if train_ds_local.target[index] == 1:
                ...
                train_ds_local.target[index] = 6
            elif train_ds_local.target[index] == 3:
                train_ds_local.target[index] = 7

    train_dl_local = data.DataLoader(dataset=train_ds_local, batch_size=args.local_bs, shuffle=True, drop_last=False)
