...
from src.client.client_fedavg_fc import *
...
for idx in range(args.num_users):
    ...
    clients.append(Client_FedAvg_FC(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep,
                                 args.lr, args.momentum, args.device, train_dl_local, test_dl_local))
...
for iteration in range(args.rounds):
    ...
    bad_user_num = args.badnum
    bad_model_range = args.badrange
    for i in range(len(w_locals)):
        wi = {}
        for k in w_locals[0].keys():
            if i < bad_user_num:
                wi[k] = torch.from_numpy(
                    bad_model_range * np.random.normal(0, 1, w_locals[i][k].cpu().numpy().shape)).to(args.device) + \
                        w_locals_before[i][k]
            else:
                wi[k] = copy.deepcopy(w_locals[i][k])
        w_locals2.append(copy.deepcopy(wi))

    for i in range(len(w_locals2)):
        w_i = {}
        for k in w_locals2[i].keys():
            # print(w_locals2[i][k].is_cuda)
            # print(w_locals_before[i][k].is_cuda)
            w_i[k] = w_locals2[i][k].to(args.device) - w_locals_before[i][k]
    ...
    ww = FedAvg(w_locals2, weight_avg=fed_avg_freqs)
    w_glob = copy.deepcopy(ww)
    net_glob.load_state_dict(copy.deepcopy(ww))
    _, acc = eval_test_fc(net_glob, args, test_dl_global)
