...
from src.backdoor_method import *
...
print(str(args))
results = []
column = [str(r) for r in range(args.num_users)]
csv_list = []
for time in range(args.times):
        ...
        for net_i in range(-1, args.num_users):
            if args.dataset == "generated":
                ...
                if args.dataset in ("mnist", 'femnist', 'fmnist'):
                    net = SimpleFC(input_dim=(28 * 28), hidden_dims=[300, 100], output_dim=10).to(args.device)
    ...
    K = args.n_basis
    # K = 5
    # for idx in range(args.num_users // 2):
    #
    ...
    input_shape = train_ds_global.data[0].shape
    mask, pattern = make_pattern(input_shape, mask_value, args.device, pattern_tensor, x_top, y_top)
    import random
    ...
    for idx in range(args.num_users):
        ...
        if idx < args.flipnum:
            for index in range(len(train_ds_local.target)):
                r = random.random()
                if r < args.backdoorfrac:
                    train_ds_local.data[index] = (1 - mask).cpu() * \
                                                 train_ds_local.data[index] + \
                                                 mask.cpu() * pattern.cpu()
                    train_ds_local.target[index] = 5
    ...
    for iteration in range(args.rounds):
        ...
        for idx in idxs_users:
            ...
            w_locals_before.append(copy.deepcopy(clients[idx].get_state_dict()))
        ...
        for idx in idxs_users:
            ...
            loss = clients[idx].train(is_print=True)
        ...
        for idx in idxs_users:
            w_locals.append(copy.deepcopy(clients[idx].get_state_dict()))
            w_locals2.append(copy.deepcopy(clients[idx].get_state_dict()))

        for i in range(len(w_locals2)):
            w_i = {}
            for k in w_locals2[i].keys():
                w_i[k] = w_locals2[i][k] - w_locals_before[i][k]
            w_local_delta.append(copy.deepcopy(w_i))

        # print(w_local_delta)
        # print(w_locals2)
        # 传入模型参数
        # fed_avg_freqs = cal_freq_by_entropy_1(w_locals2, args.lr, args.alpha)
        # 传入模型参数更新
        fed_avg_freqs = cal_freq_by_entropy_1(w_local_delta, args.lr, args.alpha, args.bound)

        # record weights
        temp_list = [0 for r in range(args.num_users)]
        bad_weights = []
        for i in range(len(idxs_users)):
            temp_list[idxs_users[i]] = fed_avg_freqs[i]
            if idxs_users[i] < args.flipnum:
                bad_weights.append(fed_avg_freqs[i])
        if not len(bad_weights) == 0:
            bad_weights_round.append(np.mean(bad_weights))
    ...
    bad_weights_avg = np.mean(bad_weights_round)
    bad_weights_var = np.var(bad_weights_round)
    with open(path + str(args.trial) + '_fnum=' + str(args.flipnum) + '_beta=' + str(
            args.beta) + '_weight_record.txt',
              'w') as text_file:
        print(bad_weights_round, file=text_file)
        print(bad_weights_avg, file=text_file)
        print(bad_weights_var, file=text_file)
    avg_result = np.mean(results)
    var_result = np.var(results)
    with open(path + str(args.trial) + '_fnum=' + str(args.flipnum) + '_beta=' + str(
            args.beta) + '_results_record_of_weight.txt',
              'w') as text_file:
        print(results, file=text_file)
        print(avg_result, file=text_file)
        print(var_result, file=text_file)
    csv_test = pd.DataFrame(columns=column, data=csv_list)
    csv_test.to_csv(path + 'times' + time + '_fnum=' + str(args.flipnum) + '_weight_record.csv')
